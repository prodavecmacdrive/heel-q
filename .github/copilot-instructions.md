...
| Shared Core | `tools/shared_core/` | — | `@heel-quest/shared-core` — canonical type definitions and utilities shared by engine + editor |
...
> **npm workspaces**: Root `package.json` declares `["engine", "tools/map_editor", "tools/shared_core"]` as workspaces. A single `npm install` from the root handles everything.

> **No test runner is configured.** TypeScript type-checking is the only automated check (`tsc`).

> **Live sync**: The editor's Vite plugin intercepts `POST /api/save-world` and writes directly to `engine/src/data/world.json`. Both servers must be running for the full edit→play loop. The `activeRoomId` field is stripped from the POST payload (editor-only state).

> **world.json is git-ignored**: It is auto-generated by the editor. Copy `world.example.json` for a fresh start.
...
### Bootstrap Flow
`main.ts` → creates canvas → instantiates `Engine` → generates placeholder textures via `PlaceholderGenerator` → fetches `world.json` from `/src/data/` → calls `WorldLoader.load()` to map editor `WorldProject` format to engine `RoomData` → registers rooms → starts game loop.

### WorldLoader (`engine/src/loaders/WorldLoader.ts`)
Single-responsibility class that converts a `WorldProject` (shared_core type) into typed `RoomData[]`. Handles entity mapping, camera extraction, door/portal mapping, asset path normalization (via `normalizeAssetPath` from shared_core), sprite sheet metadata loading (`.sheet.json` sidecars), and texture preloading. Returns `WorldLoadResult { rooms, initialRoomId, sheetMeta }`.
...
- **RoomManager**: Builds room scene from RoomData:
  ...
  - `setSheetMeta()` – receives a `Map<string, SpriteSheetMeta>` from WorldLoader; replaces the old hardcoded `SHEET_META`.
  - Sprite sheet metadata loaded from `.sheet.json` sidecar files in `assets/sprites/`.
...
- `DEPTH_SORT_SCALE = 1000` — multiplier for sprite renderOrder calculation.
...
### Type System (`types/`)
- **entities.ts**: Discriminated union – `EntityType = 'sprite' | 'animated_sprite' | 'primitive' | 'camera' | 'light' | 'sound' | 'trigger' | 'spawn' | 'door'`. Each has typed properties. `Vec3 { x, y, z }` used for JSON-safe vectors. Types are re-exported from `@heel-quest/shared-core`.
  - `TriggerEntity` – includes `shape`, `extents`, `onEnterEvent`, `onLeaveEvent`, `triggerOnce`, `conditionType` ('always' | 'item_required' | 'flag_set' | 'quest_state'), `conditionValue`, `targetEntityIds`, `payload`.
...
- `serializeForEngine()` – strips editor-only fields (`activeRoomId`) before syncing to engine.
- `saveToStorage()` – writes to localStorage **and** POSTs to `/api/save-world` (Vite plugin writes engine's `world.json`). Returns `SaveResult { localSaved, engineSynced, timestamp, error? }`.
- `loadFromStorage()` / `importFromFile()` / `downloadJSON()`.

**Persistence priority**: localStorage is always written (includes `activeRoomId` for editor state restore). The POST strips `activeRoomId`. If the POST fails, localStorage may be newer than the engine file — the UI shows a save status indicator.
...
| Panel | File | Purpose |
|---|---|---|
| **MenuBar** | `MenuBar.ts` | Logo, New/Save/Load/Export buttons, room name badge, save status indicator |
...
- **EditorGrid**: 1-unit main grid + 0.25 sub-grid + RGB axis helpers + transparent floor plane. Grid dimensions controlled by `GRID_SIZE` and `SUB_GRID_STEP` constants.
...
- **WorldMapController**: 2D vector drawing for room outlines and door segments. Vertex dragging with grid snapping (`SNAP_GRID = 0.5` units) and wall-edge snapping (`EDGE_SNAP_THRESHOLD = 2.0`). Room tool = click vertices to close polygon; Door tool = two-click line with auto room-adjacency detection.
...
assets/
  ...
  sprites/     – sprite sheets + metadata JSON + `.sheet.json` sidecar files
...
Sprite sheet metadata (columns, rows, totalFrames, stateFrames) lives in `.sheet.json` files alongside the sprite texture (e.g., `elias_sheet.sheet.json`). The engine's WorldLoader fetches these at preload time.

---

## Shared Core (`tools/shared_core/`)

Published as `@heel-quest/shared-core` via npm workspaces (local package, no registry). Contains:

- **`src/types/entities.ts`**: Canonical `EditorEntity` discriminated union, `EntityType`, `Vec3`, `TriggerCondition`, `SpriteSheetMeta`, `createDefaultEntity()` factory, `generateId()`.
- **`src/types/scene.ts`**: `WorldProject`, `RoomData`, `DoorDef`, `Vec2`, `createDefaultRoom()`.
- **`src/utils/assetPath.ts`**: `normalizeAssetPath()` — Windows backslash → forward-slash, `assets/` prefix stripping.
- **`src/index.ts`**: Barrel export of all types and utils.

Both `engine/` and `tools/map_editor/` depend on `@heel-quest/shared-core`. The editor's local `types/entities.ts` and `types/scene.ts` are now re-export shims.

**Package resolution**: Uses `"exports"` field pointing to `.ts` source files — works with Vite's `moduleResolution: "bundler"`. No build step needed.
