...
The project uses **npm workspaces**. A single install from the root handles all packages:

```bash
npm install
```

> This installs dependencies for `engine/`, `tools/map_editor/`, and `tools/shared_core/` and symlinks the shared `@heel-quest/shared-core` package.

### First-time setup

`engine/src/data/world.json` is git-ignored (it's auto-generated by the editor).  
A minimal template is provided — copy it if starting from scratch:

```bash
cp engine/src/data/world.example.json engine/src/data/world.json
```
